...
class AudioEngine:
    ...
    def resample_to_whisper(self, raw_bytes):
        ...
        try:
            ...
            # Fraction irréductible : 16000/44100 = 160/441
            gcd = math.gcd(WHISPER_RATE, self.device_rate)
            up = WHISPER_RATE // gcd  # 160
            down = self.device_rate // gcd  # 441

            resampled = resample_poly(samples, up, down)
            return np.clip(resampled, -32768, 32767).astype(np.int16).tobytes()
        except ImportError:
            # Fallback interpolation linéaire si scipy absent
            samples = np.frombuffer(raw_bytes, dtype=np.int16).astype(np.float32)
